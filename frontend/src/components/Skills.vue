      ...
      <div v-if="isLoading" class="flex animate-pulse">
        <div class="flex" v-for="n in 2" :key="'skeleton-row-' + n">
          <div
            v-for="i in 8"
            :key="'skeleton-item-' + i"
            class="bg-gray-200 dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg mx-2 md:mx-4 min-w-[120px] md:min-w-[150px] h-[136px] md:h-[168px]"
          ></div>
        </div>
      </div>
      ...
      <div v-else class="flex animate-slide-left">
        <div class="flex" v-for="n in 2" :key="'row-' + n">
          <div
            ...
            :key="skill.name + '-' + n"
            class="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg dark:border dark:border-gray-700 text-center transform hover:-translate-y-2 transition-all duration-300 mx-2 md:mx-4 min-w-[120px] md:min-w-[150px] border-2 border-transparent hover:border-orange-500/50 dark:hover:border-orange-400/50 relative overflow-hidden group hover:shadow-orange-500/30 dark:hover:shadow-orange-400/30 hover:shadow-xl"
          >
            <!-- Wave overlay -->
            <div
              class="absolute inset-0 bg-gradient-to-t from-orange-500/20 to-transparent dark:from-orange-400/20 transform translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out"
            ></div>
            <img
              :src="skill.image"
              :alt="`${skill.name} icon`"
